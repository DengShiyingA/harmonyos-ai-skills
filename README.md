...
这份知识包教会 AI 读写、审查和调试 HarmonyOS NEXT 原生应用所需的一切（约 4200 行密集、可操作的知识，237 个章节，105+ 代码示例）：
...
- **状态管理** — V1 装饰器（`@State`、`@Prop`、`@Link`、`@Provide`/`@Consume`、`@Observed` + `@ObjectLink`、`@Watch`）+ V2 装饰器（`@ComponentV2`、`@Local`、`@ObservedV2` + `@Trace`、`@Monitor`）+ StateStore 全局状态，含观察深度规则、批量更新、装饰器选择优先级
- **导航路由** — `Navigation` + `NavPathStack` 完整 API、`Router` 基础路由、`App Linking` 深链接
...
- **Kit 详细章节** — Camera Kit、Audio Kit、AVPlayer/AVRecorder、Image Kit（decode/transform/encode）、Scan Kit、Account Kit、Payment Kit、Push Kit、Map Kit、Core Vision Kit（OCR/人脸/抠图）、Form Kit（服务卡片）、AVSession Kit、Location Kit、Notification Kit、Share Kit
- **数据持久化** — relationalStore（SQLite CRUD）、preferences（KV 存储）、fileIo（文件读写）、DocumentViewPicker（文件选择器）
...
- **打包与工具** — HAP/HSP/HAR、原子化服务、DevEco Studio 6.x（hvigor）、OHPM、ArkCompiler
